...
import InteractivePieChart from '../Admin/Reports/Charts/InteractivePieChart';
import InteractiveRadarChart from '../Admin/Reports/Charts/InteractiveRadarChart';
...
import ChartSelectorCard from '../Admin/Reports/Charts/ChartSelectorCard';
import CustomChart from '../Admin/Reports/Charts/CustomChart';
import ChartEditDialog from '../Admin/Reports/Charts/ChartEditDialog';
import SurveyMapCard from '../Admin/Reports/Charts/SurveyMapCard';
import QualitativeAnalysis from '../Admin/Reports/Charts/QualitativeAnalysis';
import QualitativeComparisonAnalysis from '../Admin/Reports/Charts/QualitativeComparisonAnalysis';
import TextAnalyticsDisplay from '../Admin/Reports/Charts/TextAnalyticsDisplay';
import AIChatInterface from '../Admin/Reports/AIChatInterface';
import ReportDocumentEditor from '../Admin/Reports/ReportDocumentEditor';
import ReportLibrary from '../Admin/Reports/ReportLibrary';

const AssociationUserReports = () => {
  ...
  // State for association filtering
  const [associationOrganizations, setAssociationOrganizations] = useState([]);
  ...
  const loadData = async () => {
    try {
      console.log('🔄 AssociationUserReports.loadData called');
      ...
      // Filter for association manager
      const userRole = localStorage.getItem('userRole');
      const userStr = localStorage.getItem('user');

      if (userRole === 'association' && userStr) {
        const currentUser = JSON.parse(userStr);
        if (currentUser && currentUser.organization_id) {
          const associationId = currentUser.organization_id;
          console.log('🔄 Filtering for Association ID:', associationId);
          ...
          try {
            ...
            setAssociationOrganizations(linkedOrgs);
            console.log('🔄 Linked organizations:', linkedOrgIds);

            // Filter each category in data to only include responses from linked organizations
            Object.keys(data).forEach(key => {
              if (Array.isArray(data[key])) {
                data[key] = data[key].filter(r => linkedOrgIds.includes(r.organization_id));
              }
            });

            console.log('🔄 Filtered data for association:', {
              church: data.church?.length || 0,
              institution: data.institution?.length || 0,
              nonFormal: data.nonFormal?.length || 0
            });
          } catch (err) {
            console.error('❌ Error loading association organizations:', err);
            setError('Failed to load association organizations. Please try again.');
          }
        }
      }
      ...
      // For association managers, if the current type has no data, try to find one that does
  ...
  const generateComparisonData = async () => {
    try {
      ...
      if (!isTestMode) {
        try {
          // For association managers, we want to compare against ALL responses globally
          // but the selected response must be from their association
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
            <Box>
              <Typography variant="h4" component="h1" gutterBottom sx={{ color: adminColors.text, fontWeight: 'bold' }}>
                Association Reports - Survey Analytics
              </Typography>
              <Typography variant="body1" color="text.secondary">
                {currentAssociation ? `${currentAssociation.name} - ` : ''}Compare users from your association with global averages and analyze trends.
              </Typography>
            </Box>
      ...
      <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
          <Box>
            ...
            {associationOrganizations.length > 0 && (
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                Analyzing {associationOrganizations.length} linked organization{associationOrganizations.length !== 1 ? 's' : ''}
              </Typography>
            )}
          </Box>
              ...
              <>
                ...
                {currentAssociation && (
                  <Alert severity="info" sx={{ mb: 2 }}>
                    <Typography variant="subtitle2" gutterBottom>
                      <strong>Association Comparison Mode</strong>
                    </Typography>
                    <Typography variant="body2">
                      You are viewing users from <strong>{currentAssociation.name}</strong> and comparing them against <strong>global averages</strong> from all organizations.
                      This allows you to see how your association's organizations perform relative to the broader community.
                    </Typography>
                  </Alert>
                )}
...
export default AssociationUserReports;
